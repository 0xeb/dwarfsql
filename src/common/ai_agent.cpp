...
// Embedded documentation from prompts/dwarfsql_agent.md
#include "dwarfsql_agent_prompt.hpp"
...
void AIAgent::setup_tools() {
    libagents::Tool dwarfsql_tool;
    dwarfsql_tool.name = "dwarfsql";
    dwarfsql_tool.description =
        "Execute a SQL query against a DWARF debug database (ELF binaries with DWARF info). "
        "Available tables: compilation_units, functions, variables, types, structs, "
        "struct_members, enums, enum_values, line_info, parameters, local_variables, "
        "base_classes, calls, inlined_calls, namespaces. "
        "Example: SELECT name, low_pc, high_pc FROM functions WHERE name LIKE '%main%' ORDER BY low_pc LIMIT 10";

    dwarfsql_tool.parameters_schema = R"({
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "SQL query to execute against the DWARF database"
            }
        },
        "required": ["query"]
    })";
    ...
    dwarfsql_tool.handler = [this](const std::string& args) -> std::string {
        try {
    ...
    agent_->register_tool(dwarfsql_tool);

    if (verbose_) {
        std::cerr << "[AGENT] Registered dwarfsql tool" << std::endl;
    }
}
...
std::string AIAgent::build_primed_message(const std::string& user_message) {
    return std::string(SYSTEM_PROMPT) +
        "\n\n---\n\n"
        "# User Request\n\n"
        "Use the `dwarfsql` tool to execute SQL queries. "
        "Do not use Bash, Grep, or other tools - only use the dwarfsql tool.\n\n" +
        user_message;
}
