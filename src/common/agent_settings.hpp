...
// Agent settings stored in ~/.dwarfsql/agent_settings.json
...
// Get the settings directory path (~/.dwarfsql or %APPDATA%\dwarfsql)
inline std::string GetSettingsDir() {
#ifdef _WIN32
    char path[MAX_PATH];
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_APPDATA, nullptr, 0, path))) {
        return std::string(path) + "\\dwarfsql";
    }
    // Fallback to USERPROFILE
    const char* userprofile = std::getenv("USERPROFILE");
    if (userprofile) {
        return std::string(userprofile) + "\\.dwarfsql";
    }
    return ".dwarfsql";
#else
    ...
    if (home) {
        return std::string(home) + "/.dwarfsql";
    }
    return ".dwarfsql";
#endif
}
