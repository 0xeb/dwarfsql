...
class DwarfsqlHTTPServer::Impl {
public:
    httplib::Server svr;
    std::thread server_thread;
};

DwarfsqlHTTPServer::DwarfsqlHTTPServer() = default;

DwarfsqlHTTPServer::~DwarfsqlHTTPServer() {
    stop();
}

std::string DwarfsqlHTTPServer::queue_and_wait(const std::string& sql) {
    if (!running_.load()) {
...
int DwarfsqlHTTPServer::start(int port, HTTPQueryCallback query_cb,
    ...
    svr.Get("/", [port](const httplib::Request&, httplib::Response& res) {
        std::string welcome = "DWARFSQL HTTP Server (REPL)\n\nEndpoints:\n"
...
void DwarfsqlHTTPServer::complete_pending_commands(const std::string& result) {
...
std::string DwarfsqlHTTPServer::url() const {
